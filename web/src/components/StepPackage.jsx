...
function buildGallerySaves(saves, projectMap) {
  ...
  for (const save of saves) {
    const project = save.selected_project_id
      ? projectMap.get(save.selected_project_id)
      : null;
    const type = project?.project_type;
    const tile = {
      save,
      clientLabel:
        project?.short_client || project?.client || save.package_name,
...
export default function StepPackage({
  ...
  const existingSaveIds = new Set(
    (saves || []).map((s) => s.selected_project_id).filter(Boolean),
  );
  const existingSlugs = new Set(
    (saves || []).map((s) => s.slug).filter(Boolean),
  );

  const filteredProjects = projects
    .filter((p) =>
      formatProjectOptionLabel(p).toLowerCase().includes(search.toLowerCase()),
    )
    .map((p) => ({
      ...p,
      hasExistingSave:
        existingSaveIds.has(p.project_id) || existingSlugs.has(p.project_id),
    }))
    .sort((a, b) => {
      // Keep sort order consistent, but maybe move existing to bottom? 
      // User said "mark them", let's keep alpha sort but style them.
      return formatProjectOptionLabel(a).localeCompare(
        formatProjectOptionLabel(b),
      );
    });
    ...
    <div className="space-y-5">
            ...
            {gallery.map(({ save, clientLabel, projectTitle, projectType }) => (
              ...
              >
                ...
                <div className="truncate text-sm text-[var(--text-muted)]">
                  ...
                  {save.selected_names.length > 0
                    ? ` · ${teamPreview(save.selected_names)}`
                    : ""}
                </div>
                <a
      ...
      <div>
        <p className="text-base text-[var(--text-muted)]">
          Select the pursuit for this resume package. This determines the output folder name and reference dates.
        </p>
      </div>
      ...
      >
        <div className="relative mb-5" ref={dropdownRef}>
          <div className="mb-2 text-base font-medium text-[var(--text-main)]">
            Pursuit
          </div>
      ...
      {willOverwrite ? (
        <div className="mt-2 flex items-start gap-2 rounded-[var(--radius-sm)] border border-[var(--border-accent)] bg-[var(--bg-accent-subtle)] px-3 py-2 text-sm text-[var(--text-accent)]">
          ...
          <span>
            ...
            <span className="font-medium">Restore</span> above if you meant to
            resume it.
          </span>
        </div>
      ) : null}
