...
export default function LoginPage() {
  ...
  const isEmailAllowed = emailTrimmed ? isAllowedEmail(emailTrimmed) : false;
  const isPasswordValid = password.length >= 8;
  const isPasswordConfirmed = password === confirmPassword;
  const canSubmit = isEmailAllowed && isPasswordValid && (mode === 'signin' || isPasswordConfirmed);
    ...
    <div className="space-y-1.5">
      ...
      <input
        ...
        style={
          showStatus
            ? {
                borderColor: isValid ? '#34A853' : 'var(--accent-main)',
              }
            : undefined
        }
      />
      {showStatus && (
        <p
          style={{
            fontSize: '0.75rem',
            color: isValid ? '#34A853' : 'var(--accent-main)',
            marginTop: '0.5rem',
          }}
        >
...
function PasswordField({ label, value, onChange, matches, hasInput }) {
  return (
    <div className="space-y-1.5">
      <label
        style={{
      ...
      <input
        ...
        style={
          hasInput
            ? {
                borderColor: matches ? '#34A853' : 'var(--accent-main)',
              }
            : undefined
        }
      />
